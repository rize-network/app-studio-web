...
### **Cross-Platform Form Semantics**

`FormikForm` shares validation, field state, errors, and focus rules across web
and React Native, but its container does not have the same platform semantics:

- On web, it renders a real `<form>` and wires Formik's submit/reset handlers.
- On React Native, it renders a styled `View`. There is no submit event,
  Enter-to-submit, implicit label association, or browser validation.
- Submit and reset buttons must call `handleSubmit`/`handleReset` explicitly.
- `FormikForm autoFocus` wires single-line fields through
  `returnKeyType`/`onSubmitEditing`; multi-line inputs retain Return for newlines.
- Native labels, hints, and errors must be exposed through React Native
  accessibility props, and validation must come from Formik/Yup or application
  code rather than HTML attributes.

The rule is: keep form behavior and validation shared, but branch the platform
wiring.
...
import { FormikForm, FormikTextField, FormikCheckbox } from '@app-studio/components';
...
import { Formik } from 'formik';
import * as Yup from 'yup';
import { FormikForm, FormikTextField, FormikPassword } from '@app-studio/components';
import { Button } from '@app-studio/components';
import { Vertical } from '@app-studio/components';
...
import * as Yup from 'yup';
import { FormikForm, FormikTextField } from '@app-studio/components';
import { Button } from '@app-studio/components';
import { Vertical, Horizontal } from '@app-studio/components';
import { PlusIcon, MinusIcon } from '@app-studio/components';
...
- Preserves semantic submit/reset events on web
- Uses explicit button and return-key handlers on React Native
...
```tsx
import { useFormikInput } from '@app-studio/components';
...
**Form Structure:**
- Use semantic HTML structure on web; use accessible React Native views and
  control props on native
...
**With File Uploads (web only):**
```tsx
import { FormikForm, FormikTextField } from '@app-studio/components';
import { Uploader } from '@app-studio/components';
...
`Uploader` uses browser `File` APIs and is not exported by the React Native
build. Use a native document/image picker and place its result in Formik state
for the equivalent mobile flow.
