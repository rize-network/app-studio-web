# Button

Customizable button for user interaction

### **Import**
  ```tsx
  import { Button } from '@app-studio/web';
  ```

### **Default**
```tsx
import React from 'react';
import { Button } from '../Button';

export const DefaultButton = () => (
  <Button onClick={() => alert('Hello, World!')}>Default</Button>
);

```

### **colorScheme**
Optional string to define the color scheme of the button.

```tsx
import React from 'react';
import { Button } from '../Button';
import { Horizontal } from '../../Layout/Horizontal/Horizontal';

export const ColoredButtons = () => (
  <Horizontal gap={10}>
    {[
      'theme.primary',
      'theme.secondary',
      'theme.warning',
      'theme.success',
      'theme.error',
    ].map((color, index) => (
      <Button key={index} colorScheme={color} isAuto>
        {color}
      </Button>
    ))}
  </Horizontal>
);

```

### **loaderProps**
Optional LoaderProps object to configure the appearance and behavior of the loader animation.

```tsx
import React from 'react';
import { Button } from '../Button';
import { Vertical } from '../../Layout/Vertical/Vertical';

export const LoaderButtons = () => (
  <Vertical gap={15}>
    <Button isLoading isFilled>
      Submitting
    </Button>
    <Button
      isLoading
      loaderProps={{
        type: 'dotted',
        styles: {
          loader: { color: 'color.white' },
          text: {
            color: 'color.black',
          },
        },
      }}
      isFilled
    >
      Submitting
    </Button>
    <Button
      isLoading
      loaderPosition="right"
      loaderProps={{
        type: 'quarter',
        styles: {
          loader: { color: 'color.black' },
          text: { color: 'color.black' },
        },
      }}
      isFilled
    >
      Submitting
    </Button>
  </Vertical>
);

```

## Props

| Prop           | Type             | Description                                                                                   | Default      |
| -------------- | ---------------- | --------------------------------------------------------------------------------------------- | ------------ |
| onClick        | function         | Callback function when the button is clicked.                                                |              |
| colorScheme    | string           | The color scheme of the button.                                                               |              |
| loaderProps    | LoaderProps      | Configuration for the loader animation.                                                       |              |
| icon           | ReactNode        | Icon to be displayed within the button.                                                       |              |
| isDisabled     | boolean          | Whether the button is disabled.                                                               | false        |
| isAuto         | boolean          | Whether the button size adjusts to its content.                                               | false        |
| size           | Size             | The size of the button ('xs', 'sm', 'md', 'lg', 'xl').                                         | 'md'         |
| shadow         | Shadow/Elevation | Shadow effects for the button.                                                                |              |
| variant        | Variant          | The stylistic variation of the button ('filled', 'outline', 'link', 'ghost').                 | 'filled'     |
| effect         | string           | Interactive effects for the button ('hover', 'reverse').                                      |              |
| to             | string           | URL or path for the button to navigate to (if applicable).                                    |              |
| type           | string           | HTML button type ('button', 'submit', 'reset').                                               | 'button'     |
| isFilled       | boolean          | Whether the button has a filled background (for 'filled' and 'outline' variants).             | true         |
| isLoading      | boolean          | Whether to show a loading indicator.                                                          | false        |
| loaderPosition | string           | Position of the loader ('left', 'right').                                                     | 'left'       |
| isIconRounded  | boolean          | Whether the icon should have a rounded background.                                             | false        |
| shape          | Shape            | The shape of the button ('sharp', 'rounded', 'pillShaped').                                   | 'rounded'    |
| iconPosition   | string           | Position of the icon ('left', 'right').                                                       | 'left'       |
| styles         | CSSProperties    | Optional custom styles for the button (container, text, icon, loader).                       |              |
| className      | string           | Optional className for the button container.                                                  |              |

### **icon**
Optional React node(s) defining the icon to be displayed within the button.

```tsx
import React from 'react';
import { Button } from '../Button';
import { Center } from '../../Layout/Center/Center';

import { DustBinIcon } from '../../Icon';

import { Shape } from '../Button/Button.type';

export const IconButtons = () => (
  <Center gap={15}>
    <Button icon={<DustBinIcon size={24} />} height={48}>
      Delete
    </Button>
    <Button
      height={48}
      icon={<DustBinIcon size={24} />}
      shape={'pillShaped' as Shape}
      iconPosition="right"
      colorScheme="theme.secondary"
    >
      Delete
    </Button>
    <Button
      icon={<DustBinIcon size={24} />}
      colorScheme="theme.primary"
      isIconRounded
      isAuto
    />
  </Center>
);

```

### **isDisabled**
Optional boolean to indicate if the button is non-interactive/disabled.

```tsx
import React from 'react';
import { Button } from '../Button';

export const DisabledButton = () => (
  <Button
    onClick={() => {
      alert('Disabled');
    }}
    isDisabled
  >
    Disabled
  </Button>
);

```

### **isAuto**
Optional boolean indicating if the button size should automatically adjust to its content.

```tsx
import React from 'react';
import { Button } from '../Button';
import { Horizontal } from '../../Layout/Horizontal/Horizontal';

import { Shape } from '../Button/Button.type';

export const BorderedButtons = () => (
  <Horizontal gap={15}>
    {['sharp', 'rounded', 'pillShaped'].map((border, index) => (
      <Button key={index} shape={border as Shape} isAuto>
        {border}
      </Button>
    ))}
  </Horizontal>
);

```

### **size**
Optional Size to specify the size of the button.

```tsx
import React from 'react';
import { Button } from '../Button';
import { Vertical } from '../../Layout/Vertical/Vertical';

import { Size } from '../Button/Button.type';

export const ButtonSizes = () => (
  <Vertical gap={10}>
    {['xs', 'sm', 'md', 'lg', 'xl'].map((size, index) => (
      <Button key={index} size={size as Size}>
        Button
      </Button>
    ))}
  </Vertical>
);

```

### **shadow**
Optional shadow property that can be a predefined Shadow, Elevation value or custom CSSProperties to apply shadow effects to the button.

```tsx
import React from 'react';
import { Button } from '../Button';

export const ShadowButton = () => (
  <Button shadow={{ boxShadow: 'rgb(249, 115, 22) 0px 4px 14px 0px' }}>
    Click Me
  </Button>
);

```

### **variant**
Optional Variant to define the stylistic variation of the button.

```tsx
import React from 'react';
import { Button } from '../Button';
import { Vertical } from '../../Layout/Vertical/Vertical';

import { Variant } from '../Button/Button.type';

export const VariantButtons = () => (
  <Vertical gap={15}>
    {['filled', 'outline', 'link', 'ghost'].map((variant, index) => (
      <Button
        key={index}
        to="https://www.npmjs.com/package/app-studio"
        variant={variant as Variant}
        colorScheme="theme.primary"
        isFilled
      >
        {variant}
      </Button>
    ))}
    {['filled', 'outline', 'link', 'ghost'].map((variant, index) => (
      <Button
        key={index}
        to="https://www.npmjs.com/package/app-studio"
        variant={variant as Variant}
        isFilled
        effect="reverse"
      >
        {variant}
      </Button>
    ))}
  </Vertical>
);

```

### **effect**
Optional effect to apply interactive effects (e.g., ripple) to the button.

```tsx
import React from 'react';
import { Button } from '../../Button/Button';
import { Horizontal } from '../../Layout/Horizontal/Horizontal';

export const EffectButton = () => (
  <Horizontal gap={10}>
    <Button effect="hover">Hover</Button>
    <Button colorScheme="theme.error" variant="ghost" effect="hover">
      Hover
    </Button>
    <Button colorScheme="theme.secondary" variant="outline" effect="reverse">
      Reverse
    </Button>
    <Button colorScheme="theme.warning" variant="link" effect="reverse">
      Reverse
    </Button>
  </Horizontal>
);

```
