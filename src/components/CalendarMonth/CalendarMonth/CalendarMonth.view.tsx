...
import { CalendarMonthProps, CalendarMonthEvent, CalendarView } from './CalendarMonth.props';
import {
  layoutEvents,
  PositionedEvent,
  DAY_NAMES,
...
export const CalendarMonth: React.FC<CalendarMonthProps> = ({
  ...
  const currentMonth = useMemo(() => getFirstDayOfMonth(currentDate), [currentDate]);
  ...
  const calendarDates = useMemo(() => {
    if (currentView === 'month') {
      ...
      const weekStart = addDateDays(currentDate, -((dayOfWeek - weekStartsOn + 7) % 7));
      const dates: string[] = [];
      for (let i = 0; i < 7; i++) {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, dayIndex: number) => {
      ...
      const daysDiff = daysBetweenUTC(targetDate, dragState.originalStart.slice(0, 10));

      // Update event dates
      const newStart = addDateDays(dragState.originalStart.slice(0, 10), daysDiff);
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (dragState.isDragging) {
        ...
        const newStartDay = Math.max(0, Math.min(maxDays, dragState.startDay + daysDelta));
        ...
        if (dragState.resizeDirection === 'right') {
          ...
          const maxDays = calendarDates.length - 1;
          const newEndDay = Math.min(maxDays, dragState.startDay + newDuration - 1);
  ...
  const renderWeekView = () => (
    <>
      ...
      <View ref={gridRef} display="grid" gridTemplateColumns="repeat(7, 1fr)" minHeight={400}>
        {calendarDates.map((dateISO, index) => {
          const isDropTarget = dropTargetDays.includes(index);
          const dayEvents = eventsByDate[dateISO] || [];
        ...
        {hours.map((hour) => {
          ...
          return (
            <View
              ...
              cursor="pointer"
              style={{ '&:hover': { backgroundColor: 'color.gray.50' } }}
            >
              <View
                width={80}
                padding={8}
              ...
              <View flex={1} padding={8} position="relative" {...views.timeSlot}>
