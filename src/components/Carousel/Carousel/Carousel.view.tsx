    ...
    return (
      <>
        <View
          ...
          transform={navigationPosition === 'inside' ? 'translateY(-50%)' : undefined}
          onClick={prevSlide}
        >
          {prevButtonElement}
        </View>
  ...
  const renderIndicators = () => {
    ...
    return (
      <Horizontal
        ...
        {...(indicatorPosition === 'top' ? { top: '10px' } : { bottom: '10px' })}
      ...
      >
        {slides.map((_, index) => (
          <View
            ...
            {...(activeIndex === index ? ActiveIndicatorStyles[indicatorVariant] : {})}
            {...views?.indicator}
            {...(activeIndex === index ? views?.activeIndicator : {})}
          >
            {indicatorVariant === 'number' && (
              <Text fontSize="12px" color={activeIndex === index ? 'white' : 'inherit'}>
                {index + 1}
              </Text>
            )}
          </View>
